...
const Dashboard = () => {
  ...
  const isDesktop = useMediaQuery("(min-width: 1640px)")
  const isLaptop = useMediaQuery("(min-width: 1200px) and (max-width: 1640px)")
  ...
  const gridColumnSpan = (values: value[]) => {
    if (isDesktop) return values[0]
    if (isLaptop) return values[1]
    if (isTablet) return values[2]
    if (isMobile) return values[3]
  }
      ...
      >
        ...
        >
          ...
          <Typography
            p="0 0.6rem"
            fontSize="0.8rem"
            sx={{ color: theme.palette.secondary[200], mb: "2rem" }}
          >
